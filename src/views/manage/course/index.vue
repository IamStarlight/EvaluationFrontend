<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="课程号" min-width="150px">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.cno }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        type: [{ required: true, message: '需要选择职位', trigger: 'change' }],
        sname: [{ required: true, message: '需要输入姓名', trigger: 'change' }],
        sno: [{ required: true, message: '需要输入学号', trigger: 'blur' }],
